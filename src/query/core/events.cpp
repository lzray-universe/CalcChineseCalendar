...
std::vector<std::string> el_csv_hdr(bool calc_eclipse){
	...
	const std::array<const char*,19> solar_extra={{
		"solar_eclipse_type","solar_eclipse_mag","solar_eclipse_catalog_mag",
		"solar_eclipse_catalog_obscuration","solar_eclipse_obscuration",
		"solar_eclipse_gamma",
...
void wr_csv_sol_ecl(CsvWriter&w,const SolarEclipse&ecl,int tz_off){
	...
	wr_csv_num(w,"solar_eclipse_catalog_obscuration",ecl.catalog_obscuration);
...
void wr_csv_sol_ecl_blank(CsvWriter&w){
	...
	wr_csv_num(w,"solar_eclipse_catalog_obscuration",
			   std::numeric_limits<double>::quiet_NaN());
...
void wr_eltxt(std::ostream&os,const std::string&tz,
	...
	constexpr int kSolarEclExtraCols=19;
	...
	for(const auto&ev : events){
		...
		if(calc_eclipse){
			if(eph&&(is_full_moon_ev(ev)||ev.kind=="lunar_eclipse")){
				...
				os<<"\t";
				out_num(ecl.catalog_obscuration);
				os<<"\t";
