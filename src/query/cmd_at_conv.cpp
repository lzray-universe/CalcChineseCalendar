...
void write_at(std::ostream&os,const AtOpt&opt,const AtRes&res){
	...
	const FmtMap fmts={
		...
		{"txt",[&](){ os<<"tool=lunar format=txt type=at-batch tz_display="<<args.tz<<"\n"; os<<"line_no\tstatus\traw\till_pct\tphase_name\tlunar_date\ty_lun_gz\ty_lchun_gz\ty_rule_gz\tbazi_month\tgod_month\tgod_month_basis\td_gz\th_gz\th_true_gz\trule_profile\tyear_boundary\tmonth_boundary\tleap_month_mode\tday_boundary\tjianchu\tchong_sha\tyi\tji"; if(args.calc_eot){ os<<"\teot_minutes"; } os<<"\tmessage\n"; for(const auto&row : res.rows){ os<<row.line_no<<"\t"; if(row.ok){ os<<"ok\t"<<row.raw<<"\t"<<format_num(row.data.ill_pct)<<"\t"<<row.data.phase_name<<"\t"<<row.data.lunar_date.lun_label<<"\t"<<row.data.hli.y_lun.text<<"\t"<<row.data.hli.y_lchun.text<<"\t"<<row.data.hli.y_rule.text<<"\t"<<row.data.hli.m_gz.text<<"\t"<<row.data.hli.god_month.text<<"\t"<<row.data.hli.god_month_basis<<"\t"<<row.data.hli.d_gz.text<<"\t"<<row.data.hli.h_gz.text<<"\t"<<row.data.hli.h_gz_true.text<<"\t"<<row.data.hli.rule_profile<<"\t"<<row.data.hli.year_boundary_text<<"\t"<<row.data.hli.month_boundary_text<<"\t"<<row.data.hli.leap_month_mode_text<<"\t"<<row.data.hli.day_boundary_text<<"\t"<<row.data.hli.jianchu<<"\t"<<row.data.hli.chong_sha<<"\t"<<join_pipe(row.data.hli.yi)<<"\t"<<join_pipe(row.data.hli.ji); if(args.calc_eot){ os<<"\t"<<format_num(row.data.eot.eot_minutes); } os<<"\t\n"; }else{ os<<"error\t"<<row.raw<<"\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t"; if(args.calc_eot){ os<<"\t"; } os<<"\t"<<row.error<<"\n"; } } }},
	};
	run_fmt(fmts,args.format,"at");
}
